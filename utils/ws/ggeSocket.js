...
class GgeSocket {
    /**
     * @param {string} url           - URL do WebSocket do servidor (wss://...)
     * @param {string} serverHeader  - Zona do servidor (ex: "EmpireEx_1")
     * @param {string} username      - Nome de jogador in-game
     * @param {string} password      - Senha da conta do jogo
     * @param {number} networkId     - ID da rede GGE
     * @param {string|null} botId    - ID do documento do bot no Firestore
     */
    constructor(url, serverHeader, username, password, networkId, botId = null) {
        this.url = url;
    ...
    async connect() {
        try {
            ...
            this.socket.sendJsonCommand("lli", {
                CONM: 175, RTM: 24, ID: 0, PL: 1,
                NOM: this.username, PW: this.password,
            ...
            if (lliResponse.payload.status === 0) {
                ...
                // Conta não existe — registra via REST e tenta novamente
                const serverIndex = this.serverHeader.includes("EmpireEx_")
                    ? this.serverHeader.split("EmpireEx_")[1]
                    : "1";
                const response = await fetch(
                    `https://lp2.goodgamestudios.com/register/index.json?gameId=12&networkId=${this.networkId}&COUNTRY=FR&forceGeoip=false&forceInstance=true&PN=${this.username}&LANG=fr-FR&MAIL=&PW=${this.password}&AID=0&adgr=0&adID=0&camp=0&cid=&journeyHash=1720629282364650193&keyword=&matchtype=&network=&nid=0&placement=&REF=&tid=&timeZone=14&V=&campainPId=0&campainCr=0&campainLP=0&DID=0&websiteId=380635&gci=0&adClickId=&instance=${serverIndex}`,
                ...
                if (data.res && data.err.length === 0) {
                    ...
                    const lliResponse2 = await this.socket.waitForJsonResponse("lli");
